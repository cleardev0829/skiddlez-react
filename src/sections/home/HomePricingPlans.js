...
export default function HomePricingPlans() {
  const tempData = [
    ...
    },
    {
      ...
      titleLine2: '300$',
      subTitle: 'Per month, Billed annually',
      content: 'An expansive solution for businesses looking for top-tier service.',
      options: [
        'Hiring plan ',
    ...
    <RootStyle>
      <Container>
        <Box sx={{ mb: 10, textAlign: 'center' }}>
          <m.div>
            <Typography color="primary" component="div" variant="h2" sx={{ mb: 2 }}>
              Pricing
            </Typography>
          </m.div>
          <m.div>
    ...
    <Card sx={cardStyle === 'default' ? defualtCardStyle : secondaryCardStyle}>
      <Stack spacing={5}>
        <Stack sx={{ textAlign: 'center' }}>
          <m.div>
            <Typography sx={{ color: 'text.defualt', fontSize: '24px', fontWeight: 500 }}>
              {titleLine1} <br /> {titleLine2}
            </Typography>
          </m.div>
          <m.div>
            <Typography sx={{ color: 'text.defualt', fontSize: '12px', mb: 5, fontWeight: 400 }}>{subTitle}</Typography>
          </m.div>
          <m.div>
            <Typography sx={{ color: 'text.defualt', fontSize: '16px', fontWeight: 400 }}>{content}</Typography>
          </m.div>
        </Stack>

        <Stack sx={{ minHeight: '173px' }}>
          {options.map((option) => (
        ...
        <ButtonStyle size="large" variant="outlined" target="_blank" rel="noopener">
          Activate
        </ButtonStyle>
      </Stack>
    </Card>
